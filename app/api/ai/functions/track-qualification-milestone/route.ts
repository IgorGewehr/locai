...
export async function POST(request: NextRequest) {
  ...
  const requestId = `track_qualification_${Date.now()}_${Math.random().toString(36).substring(2, 6)}`;
  ...
  try {
    ...
    logger.info('🎯 [TRACK-QUALIFICATION-MILESTONE] Iniciando tracking', {
      requestId,
    ...
    if (!tenantId || !leadId) {
      logger.warn('⚠️ [TRACK-QUALIFICATION-MILESTONE] Campos obrigatórios faltando', {
        requestId,
        missing: {
          tenantId: !tenantId,
          leadId: !leadId
    ...
    if (typeof eventData === 'string') {
      try {
        ...
        logger.warn('⚠️ [TRACK-QUALIFICATION-MILESTONE] EventData parsing failed, usando objeto vazio', { requestId });
    ...
    // Enrich eventData with qualification-specific fields
    const enrichedEventData = {
      ...
      milestone: parsedEventData.milestone || 'qualified',
      sofiaTriggered: true,
    ...
    const contextualResponse = `Marco de qualificação registrado - dados salvos para análise`;

    logger.info('✅ [TRACK-QUALIFICATION-MILESTONE] Metric tracked', {
      requestId,
      tenantId: tenantId.substring(0, 8) + '***',
      results: {
    ...
    logger.error('❌ [TRACK-QUALIFICATION-MILESTONE] Erro no tracking', {
      requestId,
    ...
    return NextResponse.json(
      {
        success: false,
        error: 'Failed to track qualification milestone',
        requestId,
...
//   "eventData": "{\"milestone\": \"qualified\", \"timeToMilestone\": 180}"
// }
//
// With specific data:
// {
