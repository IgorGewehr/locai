...
export async function POST(request: NextRequest) {
  ...
  const requestId = `track_engagement_${Date.now()}_${Math.random().toString(36).substring(2, 6)}`;
  ...
  try {
    ...
    logger.info('💬 [TRACK-MESSAGE-ENGAGEMENT] Iniciando tracking', {
      requestId,
    ...
    if (!tenantId || !leadId) {
      logger.warn('⚠️ [TRACK-MESSAGE-ENGAGEMENT] Campos obrigatórios faltando', {
        requestId,
        missing: {
          tenantId: !tenantId,
          leadId: !leadId
    ...
    if (typeof eventData === 'string') {
      try {
        ...
        logger.warn('⚠️ [TRACK-MESSAGE-ENGAGEMENT] EventData parsing failed, usando objeto vazio', { requestId });
    ...
    // Enrich eventData with engagement-specific fields
    const enrichedEventData = {
      ...parsedEventData,
      timestamp: new Date().toISOString(),
      sessionType: parsedEventData.sessionType || 'sofia_assisted',
      outcome: parsedEventData.outcome || 'responded',
      engagementLevel: parsedEventData.engagementLevel || 'active',
      sofiaTriggered: true,
    ...
    let contextualResponse = 'Engajamento registrado';
    if (parsedEventData.responseTime) {
      contextualResponse += ` (${parsedEventData.responseTime}s)`;
    }
    contextualResponse += ' - dados salvos para análise';

    logger.info('✅ [TRACK-MESSAGE-ENGAGEMENT] Metric tracked', {
      requestId,
      tenantId: tenantId.substring(0, 8) + '***',
      results: {
    ...
    logger.error('❌ [TRACK-MESSAGE-ENGAGEMENT] Erro no tracking', {
      requestId,
    ...
    return NextResponse.json(
      {
        success: false,
        error: 'Failed to track message engagement',
        requestId,
...
//   "eventData": "{\"outcome\": \"responded\", \"responseTime\": 5.2}"
// }
//
// With specific data:
// {
