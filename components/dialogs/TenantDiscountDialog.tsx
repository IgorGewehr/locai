...
import { TenantDiscountSettings, DEFAULT_TENANT_DISCOUNT_SETTINGS } from '@/lib/types/tenant-settings';

interface TenantDiscountDialogProps {
  open: boolean;
  onClose: () => void;
  onSave: (settings: TenantDiscountSettings) => Promise<void>;
  currentSettings?: TenantDiscountSettings;
}

export default function TenantDiscountDialog({
  open,
  onClose,
  onSave,
  currentSettings
}: TenantDiscountDialogProps) {
  const [settings, setSettings] = useState<TenantDiscountSettings>(
    currentSettings || DEFAULT_TENANT_DISCOUNT_SETTINGS
  );
  ...
  useEffect(() => {
    if (currentSettings) {
      ...
      setSettings(DEFAULT_TENANT_DISCOUNT_SETTINGS);
    }
  }, [currentSettings, open]);

  const handleChange = (field: keyof TenantDiscountSettings, value: number) => {
    ...
    >
      ...
      <DialogContent dividers>
        ...
        <Alert severity="info" icon={<InfoIcon />} sx={{ mb: 3 }}>
          ...
          Estes descontos serão aplicados a TODAS as suas propriedades.
        </Alert>
