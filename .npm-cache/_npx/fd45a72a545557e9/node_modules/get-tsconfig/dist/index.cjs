"use strict";var ve=Object.defineProperty;var o=(e,t)=>ve(e,"name",{value:t,configurable:!0});var m=require("node:path"),te=require("node:fs"),de=require("node:module"),Te=require("resolve-pkg-maps"),Ae=require("fs");function E(e){return e.startsWith("\\\\?\\")?e:e.replace(/\\/g,"/")}o(E,"slash");const O=o(e=>{const t=te[e];return(s,...n)=>{const l=`${e}:${n.join(":")}`;let i=s==null?void 0:s.get(l);return i===void 0&&(i=Reflect.apply(t,te,n),s==null||s.set(l,i)),i}},"cacheFs"),B=O("existsSync"),_e=O("readFileSync"),P=O("statSync"),se=o((e,t,s)=>{for(;;){const n=m.posix.join(e,t);if(B(s,n))return n;const l=m.dirname(e);if(l===e)return;e=l}},"findUp"),z=/^\.{1,2}(\/.*)?$/,G=o(e=>{const t=E(e);return z.test(t)?t:`./${t}`},"normalizeRelativePath");function je(e,t=!1){const s=e.length;let n=0,l="",i=0,u=16,f=0,r=0,g=0,T=0,b=0;function _(c,k){let p=0,F=0;for(;p<c;){let j=e.charCodeAt(n);if(j>=48&&j<=57)F=F*16+j-48;else if(j>=65&&j<=70)F=F*16+j-65+10;else if(j>=97&&j<=102)F=F*16+j-97+10;else break;n++,p++}return p<c&&(F=-1),F}o(_,"scanHexDigits");function v(c){n=c,l="",i=0,u=16,b=0}o(v,"setPosition");function A(){let c=n;if(e.charCodeAt(n)===48)n++;else for(n++;n<e.length&&h(e.charCodeAt(n));)n++;if(n<e.length&&e.charCodeAt(n)===46)if(n++,n<e.length&&h(e.charCodeAt(n)))for(n++;n<e.length&&h(e.charCodeAt(n));)n++;else return b=3,e.substring(c,n);let k=n;if(n<e.length&&(e.charCodeAt(n)===69||e.charCodeAt(n)===101))if(n++,(n<e.length&&e.charCodeAt(n)===43||e.charCodeAt(n)===45)&&n++,n<e.length&&h(e.charCodeAt(n))){for(n++;n<e.length&&h(e.charCodeAt(n));)n++;k=n}else b=3;return e.substring(c,k)}o(A,"scanNumber");function w(){let c="",k=n;for(;;){if(n>=s){c+=e.substring(k,n),b=2;break}const p=e.charCodeAt(n);if(p===34){c+=e.substring(k,n),n++;break}if(p===92){if(c+=e.substring(k,n),n++,n>=s){b=2;break}switch(e.charCodeAt(n++)){case 34:c+='"';break;case 92:c+="\\";break;case 47:c+="/";break;case 98:c+="\b";break;case 102:c+="\f";break;case 110:c+=`
